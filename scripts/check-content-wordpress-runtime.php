...
try {
	foreach ( array( 'post', 'page' ) as $type ) {
		...
		$invalid = '<!-- wp:paragraph --><p>Missing block close.</p>';
		$r = wp_get_ability( 'content/update-' . $type )->execute( array( 'id' => $id, 'content' => $invalid, 'content_write_mode' => 'full_rebuild' ) );
		if ( ! is_wp_error( $r ) && true === ( $r['success'] ?? false ) ) { throw new RuntimeException( 'Malformed block syntax was accepted: ' . $type ); }
	...
	echo "PASS: post/page create, update and patch preserve escaped block attributes; malformed block syntax is rejected without mutation.\n";
} finally {
